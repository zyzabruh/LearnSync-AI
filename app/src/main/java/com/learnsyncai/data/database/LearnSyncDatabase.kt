...
abstract class LearnSyncDatabase : RoomDatabase() {
    ...
    /**
     * Annulation d'une notation : restaure l'état FSRS précédent, supprime
     * le log et décrémente le compteur de session — en une seule transaction.
     */
    @Transaction
    open suspend fun undoRateAtomically(
        previousCard: FlashcardEntity,
        logId: String,
        sessionId: String?
    ) {
        flashcardDao().updateFlashcard(previousCard)
        reviewLogDao().deleteReviewLogById(logId)
        if (sessionId != null) {
            reviewSessionDao().decrementCardsReviewed(sessionId)
        }
    }
    ...
    companion object {
        ...
        val MIGRATION_11_12 = object : Migration(11, 12) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE user_preferences ADD COLUMN calendarHorizonDays INTEGER NOT NULL DEFAULT 14")
                db.execSQL("ALTER TABLE user_preferences ADD COLUMN calendarStartTime TEXT NOT NULL DEFAULT ''")
                db.execSQL("ALTER TABLE user_preferences ADD COLUMN calendarDurationMinutes INTEGER NOT NULL DEFAULT 30")
                db.execSQL("ALTER TABLE user_preferences ADD COLUMN calendarReminderMinutes INTEGER NOT NULL DEFAULT 15")
            }
        }
        ...
        val MIGRATION_14_15 = object : Migration(14, 15) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE flashcards ADD COLUMN suspended INTEGER NOT NULL DEFAULT 0")
            }
        }
        ...
        val MIGRATION_15_16 = object : Migration(15, 16) {
            override fun migrate(db: SupportSQLiteDatabase) {
                ...
                db.execSQL("ALTER TABLE flashcards ADD COLUMN direction TEXT NOT NULL DEFAULT 'forward'")
                db.execSQL("ALTER TABLE flashcards ADD COLUMN typeAnswer INTEGER NOT NULL DEFAULT 0")
            }
        }

        val MIGRATION_16_17 = object : Migration(16, 17) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE flashcards ADD COLUMN sourceExcerpt TEXT NOT NULL DEFAULT ''")
            }
        }
        ...
        val MIGRATION_20_21 = object : Migration(20, 21) {
            override fun migrate(db: SupportSQLiteDatabase) {
                // Saut carte → page PDF source.
                db.execSQL("ALTER TABLE flashcards ADD COLUMN sourcePage INTEGER NOT NULL DEFAULT -1")
            }
        }
